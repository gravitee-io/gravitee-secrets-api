 ...
 * @param usesDynamicKey  when the key is computed at runtime hence true when <code>key</code> param is null
...
public record SecretSpec(
    ...
    public SecretSpec {
        ...
        ok = usesDynamicKey ^ StringUtils.hasText(key);
        if (!ok) {
            throw new IllegalArgumentException("spec either uses dynamic key or specifies one");
        }
